...
function ScoreBar({ label, score }: { label: string; score: number | null }) {
  const val = score ?? 0
  return <div className="score-row"><span className="score-label">{label}</span><div className="score-bar-track"><div className="score-bar-fill" style={{ width: `${(val / 10) * 100}%`, background: scoreBarColor(val) }} /></div><span className="score-value">{val}/10</span></div>
}
...
export default function PlacementCard({ placement: p, isNew, onUpdate }: Props) {
  ...
  const appStatus = (p.app_status ?? 'Not Applied') as AppStatus
  const priorityColor = PRIORITY_COLORS[priority]
  const priorityLabel = PRIORITY_LABELS[priority]
  const statusClass = (p.application_status ?? '').replace(/\s+/g, '-').toLowerCase()

  const patch = (changes: Partial<Placement>) => onUpdate?.({ ...p, ...changes })
  const handleStage = (e: React.ChangeEvent<HTMLSelectElement>) => {
    e.stopPropagation()
    const next = e.target.value as AppStatus
    patch({ app_status: next, date_applied: next === 'Applied' && !p.date_applied ? new Date().toISOString().slice(0, 10) : p.date_applied })
  }
  const handleField = (field: keyof Placement, value: string | null) => patch({ [field]: value } as Partial<Placement>)
  ...
  return <article className={`placement-card priority-${priority.replace(/_/g, '-').toLowerCase()} ${isNew ? 'is-new' : ''} ${expanded ? 'is-expanded' : ''}`} onClick={() => setExpanded(e => !e)}>
    ...
    <div className="pc-meta"><span className="meta-chip">{p.sector ?? '—'}</span><span className="meta-chip">{p.city ?? 'TBC'}, {p.country ?? 'TBC'}</span><span className="meta-chip">{p.placement_duration ?? 'TBC'}</span>{appStatus !== 'Not Applied' && <span className={`app-stage-chip app-stage-${appStatus.toLowerCase().replace(/\s+/g,'-')}`}>{appStatus}</span>}</div>
    ...
    <div className={`pc-details ${expanded ? 'show' : ''}`}>
      ...
      <div className="pc-detail-section application-tracking"><h4>Application Tracking</h4>
        <div className="tracking-grid">
          <label>Stage<select value={appStatus} onChange={handleStage} onClick={e => e.stopPropagation()}>{APP_STAGES.map(s => <option key={s}>{s}</option>)}</select></label>
          <label>Date applied<input type="date" value={p.date_applied ?? ''} onChange={e => handleField('date_applied', e.target.value || null)} onClick={e => e.stopPropagation()} /></label>
          <label>Interview date<input type="date" value={p.interview_date ?? ''} onChange={e => handleField('interview_date', e.target.value || null)} onClick={e => e.stopPropagation()} /></label>
          <label>CV version<input type="text" placeholder="e.g. Aerospace v4" value={p.cv_version ?? ''} onChange={e => handleField('cv_version', e.target.value || null)} onClick={e => e.stopPropagation()} /></label>
          <label>Referral / contact<input type="text" placeholder="Name or LinkedIn" value={p.referral_contact ?? ''} onChange={e => handleField('referral_contact', e.target.value || null)} onClick={e => e.stopPropagation()} /></label>
          <label>Cover letter<select value={p.cover_letter_required ?? ''} onChange={e => handleField('cover_letter_required', e.target.value || null)} onClick={e => e.stopPropagation()}><option value="">Not recorded</option><option value="Yes">Yes</option><option value="No">No</option><option value="Submitted">Submitted</option></select></label>
        </div>
        <label className="tracking-notes">Notes<textarea rows={3} placeholder="Interview notes, next steps, contacts, reminders…" value={p.notes ?? ''} onChange={e => handleField('notes', e.target.value || null)} onClick={e => e.stopPropagation()} /></label>
        <DetailItem label="Outcome" value={p.outcome}/>
      </div>
      <div className="pc-detail-section"><h4>Verification</h4><DetailItem label="Source Verified" value={p.source_verified}/><DetailItem label="Source URL" value={p.source_url}/></div>
      <div className="pc-actions">{p.application_link && <a href={p.application_link} target="_blank" rel="noopener noreferrer" className="pc-apply-btn" onClick={e => e.stopPropagation()}>{p.application_status === 'Open Now' || p.application_status === 'Open' ? 'Apply Now' : 'View Careers Page'}</a>}{p.website && <a href={p.website} target="_blank" rel="noopener noreferrer" className="pc-website-btn" onClick={e => e.stopPropagation()}>Company Website</a>}</div>
    </div>
    <button className="pc-expand" onClick={e => { e.stopPropagation(); setExpanded(e2 => !e2) }}>{expanded ? 'Show less' : 'View details'}<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className={`chevron ${expanded ? 'up' : ''}`}><polyline points="6 9 12 15 18 9" /></svg></button>
  </article>
}
