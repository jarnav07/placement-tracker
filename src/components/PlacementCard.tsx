...
export default function PlacementCard({ placement: p, isNew, onUpdate }: Props) {
  ...
  const handleAppStatusChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    onUpdate?.({ ...p, app_status: e.target.value })
  }
  ...
  return <article className={`placement-card priority-${priority.replace(/_/g, '-').toLowerCase()} ${isNew ? 'is-new' : ''} ${expanded ? 'is-expanded' : ''}`} onClick={() => setExpanded((e) => !e)}>
    ...
    <div className={`pc-details ${expanded ? 'show' : ''}`}>
      ...
      <div className="pc-detail-section"><h4>Application Tracking</h4><div className="tracking-row"><label>Status</label><select value={p.app_status ?? 'Not Applied'} onChange={handleAppStatusChange} onClick={(e) => e.stopPropagation()}><option>Not Applied</option><option>Applied</option><option>Interview</option><option>Offer</option><option>Rejected</option></select></div><DetailItem label="Verified" value={p.source_verified} /></div>
    ...
    <button className="pc-expand" onClick={(e) => { e.stopPropagation(); setExpanded((v) => !v) }}>{expanded ? 'Show less' : 'View details'}<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className={`chevron ${expanded ? 'up' : ''}`}><polyline points="6 9 12 15 18 9" /></svg></button>
  </article>
}
