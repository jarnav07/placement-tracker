  ...
  return <>
    ...
    <div className="mobile-app">
      ...
      <main className="mobile-main">
        ...
        {view === 'opportunities' && <div className="mobile-priority-row">{priorityTabs.map(tab => <button key={tab.key} className={filterPriority === tab.key ? 'active' : ''} onClick={() => setFilterPriority(tab.key)}><i style={{ background: tab.color }} />{tab.label}<small>{priorityCounts[tab.key] ?? 0}</small></button>)}</div>}
