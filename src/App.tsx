...
export default function App() {
  ...
  for (const key of Object.keys(PRIORITY_LABELS) as OverallPriority[]) {
    priorityCounts[key] = placements.filter((p) => p.overall_priority === key).length
  }

  const filtered = sortPlacements(
    placements.filter((p) => {
    ...
    <div className="app">
        ...
        <div className="header-content">
          <div className="header-top">
            <div className="brand">
              ...
              <div>
                <h1>2027-28 Placement Tracker</h1>
                <p className="brand-sub">Aerospace · Space · Rockets · F1 · {counts.total} opportunities tracked</p>
              </div>
            </div>
            <div className="header-right">
              <div className={`connection-indicator ${connected ? 'connected' : 'connecting'}`}>
                <span className="conn-dot" />
                {connected ? 'Live' : 'Connecting…'}
              </div>
              <button className="download-btn" onClick={() => downloadExcel(placements)}>
          ...
          <div className="stats-row">
            ...
            <div className="stat-chip soon"><span className="stat-num">{counts.soon}</span><span className="stat-label">Opening Soon</span></div>
      ...
      <main className="app-main">
        <section className="summary-bar">
          {PRIORITY_TABS.map((tab) => (
            <button
              key={tab.key}
              className={`summary-tab ${filterPriority === tab.key ? 'active' : ''}`}
              onClick={() => setFilterPriority(tab.key)}
              style={filterPriority === tab.key ? { borderColor: tab.color } : {}}
            >
              <span className="tab-dot" style={{ background: tab.color }} />
              <span className="tab-label">{tab.label}</span>
              <span className="tab-count">{priorityCounts[tab.key] ?? 0}</span>
            </button>
          ))}
        </section>
        ...
        <section className="controls">
          <div className="search-box">
            ...
            <input
              type="text"
              placeholder="Search companies, roles, locations, areas…"
              value={search}
        ...
        {loading ? (
          ...
          <div className="state-msg error">
            <p>Something went wrong loading placements.</p>
          ...
          <section className="placements-grid">
            {filtered.map((p) => (
              <PlacementCard key={p.id} placement={p} isNew={newIds.has(p.id)} onUpdate={updatePlacement} />
            ))}
          </section>
        )}
      </main>
      ...
      <footer className="app-footer">
        ...
        <span className="footer-live">
          <span className="footer-dot" /> Auto-updates when new roles are added or status changes
        </span>
      </footer>
    </div>
  )
