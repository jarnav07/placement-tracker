...
function containsCountry(raw: string, names: Set<string>): boolean {
  return [...names].some(name => raw === name || raw.includes(name))
}
...
export function countryGroup(country: string | null, city = '', company = ''): CountryGroup {
  ...
  // Explicit Rocket Lab NZ handling. This is deliberately checked before any
  // generic region matching because Rocket Lab has operations in multiple regions.
  if (/rocket\s*lab/.test(location) && /(new zealand|nz|auckland|christchurch|mahia|whenuapai)/.test(location)) return 'Oceania'
  if (/rocket\s*lab/.test(location) && /(long\s*beach|california|usa|united states)/.test(location)) return 'America'

  // UK must always win over Europe for mixed strings such as
  // "London, United Kingdom" or "UK / Europe".
